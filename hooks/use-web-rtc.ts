        ...
        (participantId: string, streamToAdd: MediaStream | null): RTCPeerConnection => {
            ...
            peerConnection.ontrack = (event) => {
                ...
                const existingTrack = remoteStream
                    .getTracks()
                    .find((track) => track.kind === event.track.kind && track.id === event.track.id)

                if (!existingTrack) {
                    // Add track to stream and ensure it's enabled
                    remoteStream.addTrack(event.track)
                    event.track.enabled = true

                    console.log(`[PC ${participantId}] Added ${event.track.kind} track, Stream tracks:`, {
                    ...
                    console.log(`[PC ${participantId}] Track ${event.track.kind} already exists, skipping`)
                }
            }
        ...
        (roomId: string, userName: string, stream: MediaStream) => {
            ...
            socket.on("user-muted", ({ participantId, isMuted }) => {
                ...
                setParticipants((prev) =>
                    prev.map((p) => {
                        if (p.id === participantId) {
                            // Also update the actual audio track if available
                            if (p.stream) {
            ...
            socket.on("user-video-toggled", ({ participantId, isVideoOff }) => {
                ...
                setParticipants((prev) =>
                    prev.map((p) => {
                        if (p.id === participantId) {
                            // Also update the actual video track if available
                            if (p.stream) {
