...
export function VideoParticipant({ participant, isLocal = false, className }: VideoParticipantProps) {
  ...
  useEffect(() => {
    ...
    // Force play attempt
    videoElement.play().catch((err) => {
      console.warn(`[VideoParticipant] Initial play failed for ${participant.name}:`, err)
    })
  ...
  const hasAudioTrack = participant.stream?.getAudioTracks().length > 0
  const showVideo = hasVideoTrack && !participant.isVideoOff && isVideoLoaded && !videoError
  ...
  return (
    ...
    >
      ...
      {participant.activeReaction && (
        <div className="absolute top-2 right-2 text-2xl animate-bounce">{participant.activeReaction.emoji}</div>
      )}

      {/* Error Display */}
      {videoError && (
        <div className="absolute top-2 left-2 text-xs text-red-500 bg-black/50 px-2 py-1 rounded">{videoError}</div>
      )}

      {/* Debug Info (only in development) */}
      {process.env.NODE_ENV === "development" && (
        <div className="absolute top-2 left-2 text-xs text-white bg-black/50 px-2 py-1 rounded">
          V:{hasVideoTrack ? "✓" : "✗"} A:{hasAudioTrack ? "✓" : "✗"}
          {participant.stream && ` ID:${participant.stream.id.slice(-4)}`}
        </div>
      )}
    </div>
  )
